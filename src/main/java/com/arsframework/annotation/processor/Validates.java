...
public abstract class Validates {
    ...
    public static boolean isType(Symbol.ClassSymbol symbol, Class<?>... types) {
        ...
        return superclass == null || superclass.tsym == null ? false
            : isType((Symbol.ClassSymbol)superclass.tsym, types);
    }
     ...
     *
     * @param kind 原元素类型
     * @param kinds 目标元素类型数组
     * @return true/false
     */
    public static boolean isKind(ElementKind kind, ElementKind... kinds) {
        return kind != null && kinds != null && Stream.of(kinds).anyMatch(k -> Objects.equals(k, kind));
    }
    ...
    public static boolean isEqual(Symbol.ClassSymbol clazz, Symbol.ClassSymbol other) {
        return clazz.equals(other) || (isType(clazz, byte.class, Byte.class) && isType(other, byte.class, Byte.class))
    ...
    public static JCTree.JCExpression buildNullExpression(TreeMaker maker, Names names, Symbol.VarSymbol param) {
        return param.type.isPrimitive() ? null : maker.Binary(JCTree.Tag.EQ,
            maker.Ident(names.fromString(param.name.toString())), maker.Literal(TypeTag.BOT, null));
    }
    ...
    public static JCTree.JCExpression buildNonnullExpression(TreeMaker maker, Names names, Symbol.VarSymbol... params) {
        ...
        for (Symbol.VarSymbol param : params) {
            if (!param.type.isPrimitive()) {
                JCTree.JCExpression condition = maker.Binary(JCTree.Tag.NE,
                    maker.Ident(names.fromString(param.name.toString())), maker.Literal(TypeTag.BOT, null));
    ...
    public static JCTree.JCExpression buildEmptyExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        ...
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, Map.class, Collection.class)) {
            return maker.Binary(JCTree.Tag.OR, condition,
                maker.Apply(List.nil(),
                    maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("isEmpty")),
                    List.nil()));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, String.class)) {
            JCTree.JCExpression expression = maker.Binary(JCTree.Tag.OR, condition,
                maker.Apply(List.nil(),
                    maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("isEmpty")),
                    List.nil()));
            return blank ? expression
                : maker
                    .Binary(JCTree.Tag.OR, expression,
                        maker
                            .Apply(List.nil(),
                                maker.Select(maker.Apply(List.nil(),
                                    maker.Select(maker.Ident(names.fromString(param.name.toString())),
                                        names.fromString("trim")),
                                    List.nil()), names.fromString("isEmpty")),
                                List.nil()));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, CharSequence.class)) {
            JCTree.JCExpression expression =
                maker.Binary(JCTree.Tag.OR, condition,
                    maker.Binary(JCTree.Tag.EQ, maker.Apply(List.nil(),
                        maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("length")),
                        List.nil()), maker.Literal(TypeTag.INT, 0)));
            return blank ? expression
                : maker.Binary(JCTree.Tag.OR, expression,
                    maker.Apply(List.nil(), maker.Select(
                        maker.Apply(List.nil(),
                            maker.Select(maker.Apply(List.nil(),
                                maker.Select(maker.Ident(names.fromString(param.name.toString())),
                                    names.fromString("toString")),
                                List.nil()), names.fromString("trim")),
                            List.nil()),
                        names.fromString("isEmpty")), List.nil()));
        }
        return condition;
    }
    ...
    public static JCTree.JCExpression buildFormatExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        ...
        if (isType((Symbol.ClassSymbol)param.type.tsym, String.class)) {
            ...
            if (param.type.isPrimitive()) {
                basis = maker.Apply(List.nil(),
                    maker.Select(maker.Ident(names.fromString("String")), names.fromString("valueOf")),
                    List.of(maker.Ident(names.fromString(param.name.toString()))));
            } else {
                basis = maker.Apply(List.nil(),
                    maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("toString")),
                    List.nil());
            }
        }

        // 格式校验条件表达式
        return basis == null ? null
            : merge(maker, JCTree.Tag.AND, buildNonnullExpression(maker, names, param),
                maker.Unary(JCTree.Tag.NOT, maker.Apply(List.nil(), maker.Select(basis, names.fromString("matches")),
     ...
     * @return 语法树比较条件表达式
     */
    public static JCTree.JCExpression buildNumberCompareExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        JCTree.Tag tag, long value) {
        if (isNumber((Symbol.ClassSymbol)param.type.tsym)) {
            return maker.Binary(tag, maker.Ident(names.fromString(param.name.toString())),
                maker.Literal(TypeTag.LONG, value));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, Enum.class)) {
            return maker.Binary(tag,
                maker.Apply(List.nil(),
                    maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("ordinal")),
                    List.nil()),
                maker.Literal(TypeTag.LONG, value));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, Date.class)) {
            return maker.Binary(tag,
                maker.Apply(List.nil(),
            ...
            return maker.Binary(tag,
                maker.Apply(List.nil(),
                    ...
                    List.nil()),
                maker.Literal(TypeTag.LONG, value));
        } else if (param.type.getKind() == TypeKind.ARRAY) {
            return maker.Binary(tag,
                maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("length")),
                maker.Literal(TypeTag.LONG, value));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, Map.class, Collection.class)) {
            return maker.Binary(tag,
                maker.Apply(List.nil(),
                    ...
                    List.nil()),
                maker.Literal(TypeTag.LONG, value));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, BigInteger.class)) {
            return maker.Binary(tag,
                maker.Apply(List.nil(),
                ...
                maker.Literal(TypeTag.INT, 0));
        } else if (isType((Symbol.ClassSymbol)param.type.tsym, BigDecimal.class)) {
            return maker.Binary(tag,
                maker.Apply(List.nil(),
                    maker.Select(maker.Ident(names.fromString(param.name.toString())), names.fromString("compareTo")),
    ...
    public static JCTree.JCExpression buildIsExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        ...
        return expression == null ? null
            : merge(maker, JCTree.Tag.AND, buildNonnullExpression(maker, names, param), expression);
    }
    ...
    public static JCTree.JCExpression buildOptionExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        ...
        JCTree.JCExpression expression;
        if (options.length == 0
            || (expression = buildNumberCompareExpression(maker, names, param, JCTree.Tag.NE, options[0])) == null) {
            return null;
     ...
     * @return 语法树参数验证表达式对象
     */
    public static JCTree.JCExpression buildCompareExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        String arg, JCTree.Tag tag) {
        // 参数类型是否为数字
        boolean numeric;
        // 被比较参数代码对象
        Symbol.VarSymbol target;
        ...
        if (numeric || ((tag == JCTree.Tag.EQ || tag == JCTree.Tag.NE)
        ...
        } else if (Boolean.TRUE.equals(bool) || isType((Symbol.ClassSymbol)param.type.tsym, boolean.class)) {
            if (tag == JCTree.Tag.GT) {
                ...
                expression = maker.Unary(JCTree.Tag.NOT,
                ...
                expression = maker.Binary(JCTree.Tag.AND,
                    maker.Unary(JCTree.Tag.NOT, maker.Ident(names.fromString(param.name.toString()))),
                    maker.Ident(names.fromString(arg)));
            } else {
                expression = maker.Unary(JCTree.Tag.NOT,
                    maker.Binary(JCTree.Tag.AND, maker.Ident(names.fromString(param.name.toString())),
                        maker.Unary(JCTree.Tag.NOT, maker.Ident(names.fromString(arg)))));
            }
        } else {
            expression =
    ...
    public static JCTree.JCExpression buildBoolExpression(TreeMaker maker, Names names, Symbol.VarSymbol param,
        ...
        if (isType((Symbol.ClassSymbol)param.type.tsym, boolean.class, Boolean.class)) {
            ...
            return merge(maker, JCTree.Tag.AND, buildNonnullExpression(maker, names, param),
                bool ? maker.Unary(JCTree.Tag.NOT, ident) : ident);
        }
        return null;
    }
    ...
    public static JCTree.JCIf buildValidateException(TreeMaker maker, Names names, Symbol.VarSymbol param,
        ...
        return maker.If(condition,
            maker.Throw(buildClassExpression(maker, names, exception, String.format(message, args))), null);
    }
}
